...
def download_flux_file(url, frame):
    if 'nomads' in url:
...
def download_25_file(url, frame):
    if 'nomads' in url:
...
def ingest_frame(frame,datestr,cycle,ingest_type,canadian,wind):
    ...
    if ingest_type == 'aws':
        p25_url = f'https://noaa-gfs-bdp-pds.s3.amazonaws.com/gfs.{datestr}/{cycle}/atmos/gfs.t{cycle}z.pgrb2.0p25.f{name_frame(frame)}'
        flux_url = f'https://noaa-gfs-bdp-pds.s3.amazonaws.com/gfs.{datestr}/{cycle}/atmos/gfs.t{cycle}z.sfluxgrbf{name_frame(frame)}.grib2'
    elif ingest_type == 'nomads':
        flux_url = f'https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_sflux.pl?file=gfs.t{cycle}z.sfluxgrbf{name_frame(frame)}.grib2&lev_surface=on&var_PRATE=on&subregion=&leftlon=230&rightlon=300&toplat=60&bottomlat=25&dir=%2Fgfs.{datestr}%2F{cycle}%2Fatmos'
        # p25_url = f'https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl?file=gfs.t{cycle}z.pgrb2.0p25.f{name_frame(frame)}&lev_1000_mb=on&lev_500_mb=on&lev_700_mb=on&lev_850_mb=on&lev_925_mb=on&var_HGT=on&var_TMP=on&var_UGRD=on&var_VGRD=on&leftlon=230&rightlon=300&toplat=60&bottomlat=25&dir=%2Fgfs.{datestr}%2F{cycle}%2Fatmos'
        # p25_url = f'https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl?file=gfs.t{cycle}z.pgrb2.0p25.f{name_frame(frame)}&lev_1000_mb=on&lev_500_mb=on&lev_700_mb=on&lev_850_mb=on&lev_925_mb=on&var_HGT=on&var_RH=on&var_TMP=on&var_UGRD=on&var_VGRD=on&leftlon=230&rightlon=300&toplat=60&bottomlat=25&dir=%2Fgfs.{datestr}%2F{cycle}%2Fatmos'
        p25_url = f'https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl?file=gfs.t{cycle}z.pgrb2.0p25.f{name_frame(frame)}&lev_0C_isotherm=on&lev_1000_mb=on&lev_500_mb=on&lev_700_mb=on&lev_850_mb=on&lev_925_mb=on&var_HGT=on&var_TMP=on&var_UGRD=on&var_VGRD=on&subregion=&leftlon=230&rightlon=300&toplat=60&bottomlat=25&dir=%2Fgfs.{datestr}%2F{cycle}%2Fatmos'

    download_25_file(p25_url,frame)
    if int(frame) > 0:
        download_flux_file(flux_url,frame)

    if canadian == True and frame%30 == 0:
        if frame+30 > max_frame:
            max_ingest_frame = max_frame
        else:
            max_ingest_frame = frame+31
...
with open('model_config.txt', 'r') as myfile:
    ...
    datestr = text[9]
    cycle = text[10]
    plot_types = text[11].split(',')
    if text[12] == 'True':
        plot_counties = True
    else:
        plot_counties = False
    domain = text[13].split(',')
...
for frame in range(first_frame,max_frame+1,step):
    ...
    for plot_type in plot_types:
        ...
        cbar = plt.colorbar(cf, ax=ax, orientation='horizontal', pad=0.05, label=plot_label)
    ...
    for n in range(len(points)):
        ...
        #sort df by forecast_hour
        df = df.sort_values(by=['forecast_hour'])
        #new df column that adds forecast_hour to init_datetime
        ...
        plt.subplots_adjust(hspace=0.25,left=0.05, right=0.925, top=0.9, bottom=0.05)
        ...
        for halfway_acc_snow_value, halfway_tick_time in zip(halfway_acc_snow_values, halfway_tick_times):
            plt.text(halfway_tick_time,max_tick-1,str(round(halfway_acc_snow_value,1)),ha='center',va='top',fontsize=10,color='blue',fontweight='bold')
        ...
        if wind == True:
            plt.title('Temperature (Red) / SLR (Black)')
        else:
        ...
        plt.gcf().get_axes()[1].tick_params(axis='x', which='major', labelsize=9, rotation=45)
        plt.gcf().get_axes()[3].tick_params(axis='x', which='major', labelsize=9, rotation=45)

        init_label = init_datetime.strftime('%m/%d %Hz')

        plt.suptitle("Point Forecast: "+name+" (Grid Elevation: "+str(round(df['elevation'][0]))+"') - Init: "+init_label, fontsize=16)
