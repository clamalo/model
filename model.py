...
# create colormap for wind plottingß
def wind_colormap():
...
def ingest_frame(frame,datestr,cycle,ingest_source,canadian,compute_wind):
    ...
    if frame%30 == 0:
        ...
        threads = [threading.Thread(target=download_25_file, args=(frame,ingest_source,compute_wind)) for frame in frames]
        ...
        if frames[0] == 0:
            frames = frames[1:]
        threads = [threading.Thread(target=download_flux_file, args=(frame,ingest_source)) for frame in frames]
        for thread in threads:
            thread.start()
        for thread in threads:
            thread.join()
    ...
    if canadian == True and frame%30 == 0:
        if frame+(step*10) > max_frame:
            ...
            max_ingest_frame = frame+(step*10)+1
        frames = range(frame,max_ingest_frame,step)
        threads = [threading.Thread(target=download_canadian_file, args=(frame,compute_wind)) for frame in frames]
        for thread in threads:
            thread.start()
        for thread in threads:
            thread.join()
...
for frame in range(first_frame,max_frame+1,step):
    ...
    # crop the datasets to the desired domain
    if ingest_source == 'aws':
        reg_ds = reg_ds.sel(latitude=slice(domain[0],domain[2]),longitude=slice(domain[1],domain[3]))
    elif ingest_source == 'nomads':
    ...
    if frame > first_frame:
        ...
        # crop the dataset to the desired domain
        if ingest_source == 'aws':
    ...
    if canadian == True:
        ...
        # convert the longitude to match the other datasets
    ...
    # if the user requests to compute with canadian data, interpolate the canadian data to the same grid as the american data
    if canadian == True:
        canadian_ds = canadian_ds.interp(latitude=sp_ds.latitude, longitude=sp_ds.longitude)

    # interpolate the low resolution upper air data to the same grid as the surface data, and copy the surface data precip to the main
    ...
    # interpolate the dataset to the elevation dataset, add elevation as a variable, and drop the elevation variable from the elevation dataset
    ...
    for n in range(len(points)):
        ...
        timezone_str = obj.timezone_at(lng=lon-360, lat=lat)
