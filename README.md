# Model Documentation - updated March 7, 2023
...
The format for a point is ```name,latitude,longitude,region,tag```. Make sure that the longitude is in a -180-180 format. Be sure that there are no extra/empty lines in the file. The major resorts in my ```points.txt``` file are tagged with "Work", meaning if I set the ```points_tag``` variable in ```model_config.txt``` as "Work", it will only plot these points. If you don't want to assign a tag to a point, you can leave the tag blank, but there should be four commas in every line. Make sure that there is no blank extra line at the end of the points file.
